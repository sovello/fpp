...
public class Prog4 {
	public static void main(String[] args) {
		...
		String row = ""; //initialize row to an empty string
		for( int i = 0; i< records.length(); i++) {			
			row += records.charAt(i); //concat row until a row is complete
			if (records.charAt(i) == ':') {
				System.out.println(row.substring(0, row.indexOf(','))); //print the ID part
				row = ""; //reset the row to an empty string
			}
		}
	}
}
